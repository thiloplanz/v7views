...
public class BSONBackedObjectLoader {
	...
	public static Iterable<BSONBackedObject> wrapIterable(
			final Iterable<? extends BSONObject> iterable,
			final SchemaDefinition sd) {
		return new Iterable<BSONBackedObject>() {

			public Iterator<BSONBackedObject> iterator() {
				return wrapIterator(iterable.iterator(), sd);
			}

		};
	}

	public static Iterator<BSONBackedObject> wrapIterator(
			final Iterator<? extends BSONObject> iterator,
			final SchemaDefinition sd) {
		return new Iterator<BSONBackedObject>() {

			public boolean hasNext() {
				return iterator.hasNext();
			}

			public BSONBackedObject next() {
				return wrap(iterator.next(), sd);
			}
